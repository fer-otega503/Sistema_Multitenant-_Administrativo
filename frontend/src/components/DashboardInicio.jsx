 ...
 */
const DashboardInicio = ({ onLogout }) => {
  const navigate = useNavigate();

  // Obtener sesión actual guardada en TokenService / localStorage
  const sessionUser = TokenService.getUserSession();

  // Estados para usuario, rol, negocio y métricas
  ...
  useEffect(() => {
    ...
    // Verificación estricta: los roles Admin, Administrador y Gestor pueden ingresar
    const normalizedRole = activeRole.toLowerCase();
    if (!normalizedRole.includes('admin') && !normalizedRole.includes('gestor')) {
      setIsAdmin(false);
      setIsLoading(false);
      return;
    }

    // Etiqueta del rol mostrada en el Header
    ...
    // 2. Inyectar y consumir datos desde el Backend
    const fetchMetrics = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/dashboard/metrics', {
          headers: {
            'x-tenant-id': sessionUser?.tenantId || 'ferreteria'
          }
        });
        ...
        console.warn('Backend offline o usando datos iniciales de fallback:', error);
        // Fallback a los datos de ejemplo solicitados para Ferretería
        setMetricas({
          ventasDia: '$ 370.0000',
          ventasMes: '$ 1540.0000',
          inventario: '685.0000'
  ...
  // 🛡️ CONTROL DE ACCESO: Si el usuario NO es Administrador
  if (!isAdmin) {
    return (
      <div style={styles.deniedContainer}>
        <div style={styles.deniedCard}>
          ...
          <p style={styles.deniedSubtext}>
            Tu rol actual detectado es: <em>{sessionUser?.role || 'Empleado'}</em>.
          </p>
